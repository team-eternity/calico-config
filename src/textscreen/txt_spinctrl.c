...
static unsigned int SpinControlWidth(txt_spincontrol_t *spincontrol)
{
    ...
    switch (spincontrol->type)
    {
        ...
        case TXT_SPINCONTROL_DOUBLE:
            minw = FloatWidth((float)spincontrol->min.d, (float)spincontrol->step.d);
            maxw = FloatWidth((float)spincontrol->max.d, (float)spincontrol->step.d);
            break;
...
static void SetBuffer(txt_spincontrol_t *spincontrol)
{
    ...
    switch (spincontrol->type)
    {
        ...
        case TXT_SPINCONTROL_DOUBLE:
            FloatFormatString((float)spincontrol->step.d, format, sizeof(format));
            TXT_snprintf(spincontrol->buffer, spincontrol->buffer_len,
                         format, spincontrol->value->d);
            break;
    }
}
...
static void EnforceLimits(txt_spincontrol_t *spincontrol)
{
    switch (spincontrol->type)
    {
        ...
        case TXT_SPINCONTROL_DOUBLE:
            if (spincontrol->value->d > spincontrol->max.d)
                spincontrol->value->d = spincontrol->max.d;
            else if (spincontrol->value->d < spincontrol->min.d)
                spincontrol->value->d = spincontrol->min.d;
            break;
    }
}
...
static void FinishEditing(txt_spincontrol_t *spincontrol)
{
    if (spincontrol->editing)
    {
        switch (spincontrol->type)
        {
            ...
            case TXT_SPINCONTROL_DOUBLE:
                spincontrol->value->d = atof(spincontrol->buffer);
                break;
        }
        ...
        TXT_EmitSignal(&spincontrol->widget, "changed"); // haleyjd
    }
}
    ...
    else
    {
        ...
        if (key == KEY_LEFTARROW)
        {
            switch (spincontrol->type)
            {
                ...
                case TXT_SPINCONTROL_DOUBLE:
                    spincontrol->value->d -= spincontrol->step.d;
                    break;
            }
        ...
        if (key == KEY_RIGHTARROW)
        {
            switch (spincontrol->type)
            {
                ...
                case TXT_SPINCONTROL_DOUBLE: // haleyjd
                    spincontrol->value->d += spincontrol->step.d;
                    break;
            }
...
txt_spincontrol_t *TXT_NewDoubleSpinControl(double *value, double min, double max)
{
    txt_spincontrol_t *spincontrol;

    spincontrol = TXT_BaseSpinControl();
    spincontrol->type = TXT_SPINCONTROL_DOUBLE;
    spincontrol->value = (void *) value;
    spincontrol->min.d = min;
    spincontrol->max.d = max;
    spincontrol->step.d = 0.1;
