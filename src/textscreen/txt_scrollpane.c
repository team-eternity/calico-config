...
static void TXT_ScrollPaneSizeCalc(TXT_UNCAST_ARG(scrollpane))
{
    ...
    if (scrollpane->child != NULL)
    {
        if (scrollpane->child->w < (unsigned int)scrollpane->w)
        {
            scrollpane->child->w = scrollpane->w;
        }
        if (scrollpane->child->h < (unsigned int)scrollpane->h)
        {
...
void TXT_ScrollPaneShowSelectedWidget(txt_scrollpane_t *scrollpane)
{
    TXT_ScrollPaneLayout(scrollpane);
    ShowSelectedWidget(scrollpane);
}
