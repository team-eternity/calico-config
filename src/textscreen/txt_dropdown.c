                                 ...
                                 void *unused)
{
    ...
    if (x < win->x || (unsigned int)x > win->x + win->w || y < win->y || (unsigned int)y > win->y + win->h)
    {
        TXT_CloseWindow(window);
        return 1;
    }
...
static void OpenSelectorWindow(txt_dropdown_list_t *list)
{
    ...
    const int above_threshold = (list->num_values >= SCROLL_THRESHOLD);
    ...
    TXT_SetWindowPosition(window, TXT_HORIZ_LEFT, TXT_VERT_TOP,
                          list->widget.x - 2, above_threshold ? SCROLL_Y : SelectorWindowY(list));

    void *parent_widget = window;
    txt_scrollpane_t *pane = NULL;

    // haleyjd: use a scrollpane if the list is too large
    if (above_threshold)
    {
        const int dd_width = DropdownListWidth(list);
        txt_table_t *const table = TXT_NewTable(1);
        TXT_SetColumnWidths(table, dd_width);

        pane = TXT_NewScrollPane(0, SCROLL_THRESHOLD, table);
        TXT_AddWidget(window, pane);
        parent_widget = table;
    }
    ...
    for (i=0; i<list->num_values; ++i)
    {
        ...
        TXT_AddWidget(parent_widget, button);
        ...
        if (i == *list->variable)
        {
            TXT_SelectWidget(parent_widget, button);
            if (pane)
            {
                TXT_ScrollPaneShowSelectedWidget(pane);
            }
        }
    }
