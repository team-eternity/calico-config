...
extern txt_widget_class_t txt_dbl_inputbox_class;
...
static void SetBufferFromValue(txt_inputbox_t *inputbox)
{
    ...
    else if (inputbox->widget.widget_class == &txt_dbl_inputbox_class)
    {
        double *value = (double *) inputbox->value;
        TXT_snprintf(inputbox->buffer, inputbox->buffer_len, "%.2f", *value);
    }
}
...
static void StartEditing(txt_inputbox_t *inputbox)
{
    ...
    if (inputbox->widget.widget_class == &txt_int_inputbox_class ||
        inputbox->widget.widget_class == &txt_dbl_inputbox_class)
    {
        TXT_StringCopy(inputbox->buffer, "", inputbox->buffer_len);
    }
    else
...
static void FinishEditing(txt_inputbox_t *inputbox)
{
    ...
    if (inputbox->widget.widget_class == &txt_inputbox_class)
    {
        ...
        *((char **) inputbox->value) = estrdup(inputbox->buffer);
    }
    else if (inputbox->widget.widget_class == &txt_int_inputbox_class)
    {
    ...
    {
        *((double *) inputbox->value) = atof(inputbox->buffer);
    }
...
static void TXT_InputBoxDrawer(TXT_UNCAST_ARG(inputbox))
{
    ...
    if (TXT_UTF8_Strlen(inputbox->buffer) > (unsigned int)(w - 1))
    {
...
txt_widget_class_t txt_dbl_inputbox_class =
{
    TXT_AlwaysSelectable,
    TXT_InputBoxSizeCalc,
    TXT_InputBoxDrawer,
                                   ...
                                   void *value, int size)
{
    ...
    inputbox->buffer = ecalloc(char, inputbox->buffer_len, sizeof(char));
    inputbox->editing = 0;
...
txt_inputbox_t *TXT_NewDoubleInputBox(double *value, int size)
{
    return NewInputBox(&txt_dbl_inputbox_class, value, size);
}
